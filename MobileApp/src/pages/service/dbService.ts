...
export class DBService {
    ...
    constructor(private sqlite: SQLite,private jobService:JobService,private siteService:SiteService,public employeeService:EmployeeService, public assetService:AssetService) {
    ...
    setAsset()
    {
        ...
        setTimeout(()=>{
            this.create(tablename,createQuery,insertQuery,updateQuery,param)
        },15000)
    ...
    setJobs()
    {
        setTimeout(()=>{
                ...
                this.jobService.getJobs(search).subscribe(
                    response => {
                        ...
                        if(jobs)
                        {
                            ...
                            {
                                param.push([jobs[i].id,jobs[i].assetId,jobs[i].title,jobs[i].employeeName,jobs[i].siteName,jobs[i].plannedEndTime,jobs[i].plannedStartTime,jobs[i].description,jobs[i].status,jobs[i].maintenanceType])
                            }
                        }
                    },
                    error => {
            ...
            var tablename = 'job'
            var createQuery = "create table if not exists job(id INT,assetId INT,title TEXT,employeeName TEXT,siteName TEXT,plannedEndTime TEXT,plannedStartTime TEXT,description TEXT,status TEXT,maintenanceType TEXT)";
            var insertQuery = "insert into job(id,assetId,title,employeeName ,siteName,plannedEndTime,plannedStartTime,description,status,maintenanceType) values(?,?,?,?,?,?,?,?,?,?)";
            var updateQuery = "update job set assetId,title,employeeName ,siteName,plannedEndTime,plannedStartTime,description,status,maintenanceType where id=? ";
            setTimeout(()=>{
                this.create(tablename,createQuery,insertQuery, updateQuery,param)
            },15000)
        },3000)
