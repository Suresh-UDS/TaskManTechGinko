...
public class JobSpecification implements Specification<Job> {
	...
	public Predicate toPredicate(Root<Job> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
		...
		// Predicate firstStage = builder.and(predicates.toArray(new
		...
		if (orPredicates.size() > 0) {
			// if(searchCriteria.getSiteId() > 0) {
			// finalExp = builder.and(orPredicates.toArray(new
			// Predicate[orPredicates.size()]));
			// }else {
		...
		// predicates.add(builder.equal(root.get("site.id"),
		// searchCriteria.getSiteId()));
