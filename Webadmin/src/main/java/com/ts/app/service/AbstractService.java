...
import com.ts.app.web.rest.JobManagementResource;
...
//	@Inject
//	private JobManagementResource jobManagementResource;
...
//******************************************Modified by Vinoth***********************************************************************
	...
	protected void sendJobCompletionNotifications(Employee ticketOwner, Employee assignedTo, Employee currentUserEmp, Job job, Ticket ticket, Site site, boolean isNew,String subject,String content,String filename, boolean isHtml,ByteArrayOutputStream pdfContent, Map<String, String> env) {
		Hibernate.initialize(assignedTo.getUser());
		User assignedToUser = assignedTo.getUser();
		Hibernate.initialize(ticketOwner.getUser());
		User ticketOwnerUser = ticketOwner.getUser();
		...
		Setting jobCheckList = null;
		settings = settingsRepository.findSettingByKeyAndSiteIdOrProjectId(SettingsService.EMAIL_NOTIFICATION_JOBCHECKLIST, site.getId(), site.getProject().getId());
		if(CollectionUtils.isNotEmpty(settings)) {
			jobCheckList = settings.get(0);
		}
		Setting jobCheckListEmails = null;
		if(jobCheckList != null && jobCheckList.getSettingValue().equalsIgnoreCase("true")) {
		settings = settingsRepository.findSettingByKeyAndSiteIdOrProjectId(SettingsService.EMAIL_NOTIFICATION_JOBCHECKLIST_EMAILS, site.getId(), site.getProject().getId());
		if(CollectionUtils.isNotEmpty(settings)) {
			jobCheckListEmails = settings.get(0);
		}
		}
	    ...
	    String ticketOwnerEmail = (ticketOwnerUser != null ? "," + (StringUtils.isNotEmpty(ticketOwnerUser.getEmail()) ? ticketOwnerUser.getEmail() : "") : "");
	    String ticketEmails = ticketReportEmails != null ? ticketReportEmails.getSettingValue() : "";
	    String jobEmails = jobCheckListEmails != null ? jobCheckListEmails.getSettingValue() : "";
		assignedToEmail += Constants.COMMA_SEPARATOR + ticketEmails;
		ticketOwnerEmail += Constants.COMMA_SEPARATOR + ticketEmails;
		
		assignedToEmail += Constants.COMMA_SEPARATOR + jobEmails;
		ticketOwnerEmail += Constants.COMMA_SEPARATOR + jobEmails;
	    if(StringUtils.isNotEmpty(ticket.getStatus()) && (job.getStatus().equals(JobStatus.COMPLETED))) {
//    			if(assignedTo != null) {
//			    mailService.sendJobCompletionMail(ticketUrl,jobUrl,assignedTo.getUser(),assignedToEmail,site.getName(),ticket.getId(), String.valueOf(ticket.getId()),
//	        				assignedToUser.getFirstName(), assignedTo.getName(), currentUserEmp.getName(), currentUserEmp.getEmpId(), ticket.getTitle(),ticket.getDescription(), job.getStatus().name(), job.getId(), job.getTitle());
//			   // mailService.sendJobCheckListEmail(assignedToEmail, subject, content, isHtml, filename, pdfContent);
//			   // mailService.sendEmail(assignedToEmail, subject, content, isHtml, filename, pdfContent);
//    			}
//    			
//		    mailService.sendJobCompletionMail(ticketUrl,jobUrl,ticketOwner.getUser(),ticketOwnerEmail,site.getName(),ticket.getId(), String.valueOf(ticket.getId()),
//    				assignedToUser.getFirstName(), assignedTo.getName(), currentUserEmp.getName(), currentUserEmp.getEmpId(), ticket.getTitle(),ticket.getDescription(), job.getStatus().name(), job.getId(), job.getTitle());
